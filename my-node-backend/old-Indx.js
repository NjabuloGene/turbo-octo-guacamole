...
// Add to your index.js - PAYFAST INTEGRATION
...
// Create PayFast payment (Initiate payment)
app.post('/api/payfast/create', verifyToken, async (req, res) => {
    try {
        ...
        const payfastData = {
            ...
            merchant_key: process.env.fbpag8idlwqOi,
            return_url: `${process.env.FRONTEND_URL}/payment-success.html`,
            cancel_url: `${process.env.FRONTEND_URL}/payment-cancel.html`,
            notify_url: `${process.env.FRONTEND_URL}/api/payfast/ipn`,  // Important for webhook
            amount: amount.toFixed(2),
            item_name: item_name,
            item_description: item_description.substring(0, 255),
            custom_str1: job_id,
            custom_str2: candidate_id,
            custom_str3: req.user.id,
            custom_int1: orderId,
            email_confirmation: 1,
            confirmation_address: req.user.email
        };

        // Add passphrase only if it exists
if (process.env.PAYFAST_PASSPHRASE) {
    payfastData.passphrase = process.env.PAYFAST_PASSPHRASE;
}
        ...
        await pool.query(
            ...
            [job_id, req.user.id, candidate_id, amount, orderId]
        );
        ...
        // Return the form data to frontend
        res.json({
            success: true,
...
// PayFast Instant Payment Notification (IPN) webhook
app.post('/api/payfast/ipn', express.urlencoded({ extended: true }), async (req, res) => {
    try {
        ...
        // Verify the payment (you should also verify the signature)
        const payment_status = ipnData.payment_status;
        const orderId = ipnData.custom_int1;
        const amount = ipnData.amount;
        const jobId = ipnData.custom_str1;
        const candidateId = ipnData.custom_str2;
        const employerId = ipnData.custom_str3;
...
// Release payment to candidate (after job completion)
app.post('/api/payfast/release', verifyToken, async (req, res) => {
    try {
        ...
        // Get escrow details
        const escrow = await pool.query(
            `SELECT * FROM escrow_transactions WHERE id = $1 AND status = 'held'`,
            [escrow_id]
        );
